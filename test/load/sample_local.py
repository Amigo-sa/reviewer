...
# default number of docs (used if not present in to_fill
DEFAULT_FILL_AMT = 10
...
def fill_db(db, to_fill, validate_after_fill, diverse):
        ...
        for col_name, field_list in doc_fields.items():
            ...
            if can_fill:
                ...
                if col_name in to_fill:
                    ...
                    insert_amount = DEFAULT_FILL_AMT
                increment = 1
                ...
                for i in range(insert_amount):
                    ...
                    if col_name == "person":
                        auth_doc = {"_id": int_to_obj_id(doc_ctr + \
                                    (DEFAULT_FILL_AMT if not ("person" in to_fill) else to_fill["person"]))}
                        auth_doc.update({
                            "attempts": 0,
                            "auth_code": None,
                            "is_approved": True,
...
if __name__ == "__main__":
    drop_collections(db)
    fill_db(db,to_fill,validate_after_fill,diverse)
